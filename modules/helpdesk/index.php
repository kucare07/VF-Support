...
// สามารถเพิ่มเงื่อนไขการค้นหาตรงนี้ได้ในอนาคต
$where_clauses = [];
$params = [];
...
function renderMiniStepper($current_status)
{
    ...
    if ($current_index === false) $current_index = 0; // Fallback

    $html = '<div class="mini-stepper" title="สถานะ: ' . ucfirst($current_status) . '">';
    foreach ($steps as $index => $step) {
        $class = '';
        if ($index < $current_index) {
            $class = 'completed'; // ผ่านมาแล้ว
        } elseif ($index == $current_index) {
            $class = 'active';    // อยู่ตรงนี้
        }
...
<style>
    ...
    /* --- Full Stepper (สำหรับหน้า View Modal) --- */
    .stepper-wrapper {
        display: flex;
    ...
    /* Progress Line Fill Logic (Optional CSS-only trick usually requires complex selectors, we will handle active fill with JS or simple static classes) */
</style>
